...
## Session: 2026-01-06 (Infrastructure & Large Video Upload Fixes)

### Issues Addressed

1. **Settings Page Mobile Responsiveness**
   - Connection cards were overflowing on mobile screens (buttons cut off)
   - Fixed by making card layout stack vertically on mobile (`flex-col sm:flex-row`)
   - Added `flex-wrap` to button containers
   - Added `break-words` for long usernames (e.g., TikTok open_id)

2. **Next.js Vulnerability**
   - Vercel build failed: "Vulnerable version of Next.js detected"
   - Updated Next.js from `16.0.3` → `16.1.1`
   - Updated eslint-config-next to match

3. **TikTok Large Video Upload Failure**
   - Error: "The chunk size is invalid"
   - Root cause: Chunk size was set to full video size (115MB), exceeding TikTok's limits
   - Fixed by implementing proper 10MB chunking in `tiktokClient.ts`

4. **Instagram & X Upload Timeouts**
   - Large videos (110MB+) were timing out on Vercel Hobby plan (60s limit)
   - X requires ~22 sequential chunk uploads for large videos
   - Instagram video processing polling can take 2+ minutes

### Solution: Inngest Background Processing

Implemented **Inngest** for background job processing to bypass Vercel's 60-second timeout on Hobby plan.

**New files:**
- `app/src/lib/inngest.ts` - Inngest client configuration
- `app/src/server/jobs/inngest-functions.ts` - Background publish function
- `app/src/app/api/inngest/route.ts` - Inngest webhook endpoint

**Modified files:**
- `app/src/server/jobs/posting.ts` - Added `createPostJobOnly()` for creating job records without executing
- `app/src/app/api/posts/route.ts` - Now triggers Inngest background job instead of synchronous execution

**How it works:**
1. User submits post → API creates job records with "in_progress" status
2. API triggers Inngest event `post/publish.requested`
3. API returns immediately with "Publishing in progress..."
4. Inngest runs the actual uploads in the background (no timeout limit)
5. Job status updates to "completed" or "failed" when done

**Inngest Setup:**
- Free tier: 25,000 runs/month
- Connected via Vercel integration (auto-adds `INNGEST_SIGNING_KEY` and `INNGEST_EVENT_KEY`)
- Dashboard: https://app.inngest.com

### Environment Variables Added (via Vercel Integration)
...
Updated `vercel.json` to set 300s timeout for posts route (applies to Pro plan; Hobby plan capped at 60s, hence Inngest solution):
...
| Platform | Media Type | Size Limit | Notes |
|----------|-----------|------------|-------|
| TikTok | Video only | ~4GB | 10MB chunks, sandbox requires private account |
| YouTube | Video only | Large | Supports location metadata |
| X (Twitter) | Image/Video | 512MB video | 5MB chunks, OAuth 1.0a |
| Instagram | Image/Video | ~100MB | REELS for video, polling for processing |
| LinkedIn | Image/Video | Large | Assets API for organic posts |
| Facebook Page | Image only | - | Video support not yet implemented |
| Google Business Profile | Image only | 25MB | Video not supported via API |

### Commits

- `93748a2` - fix: mobile responsiveness for settings page connection cards
- `73c4c79` - fix: update Next.js to 16.1.1 to resolve vulnerability
- `2879b0c` - fix: TikTok chunked upload for large videos (10MB chunks)
- `bd6df62` - fix: increase posts API timeout to 300s for large video uploads
- `7112c03` - feat: add Inngest for background video uploads (no timeout limit)
